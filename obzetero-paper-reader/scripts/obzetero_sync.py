...
INTENSIVE_READING_TAGS = {"待阅读", "马上读", "阅读中"}
...
def is_intensive_reading_tag(tag: str) -> bool:
    return tag.lstrip("/") in INTENSIVE_READING_TAGS
...
def write_index(papers: list[Paper], vault: Path, collection: str, state: dict[str, Any]) -> None:
    ...
    for paper in papers:
        ...
        note = paper_note_path(vault, collection, paper)
        item_state["title"] = paper.title
        item_state["collection"] = collection
        if note.exists():
            item_state["note_path"] = str(note)
        else:
            item_state.pop("note_path", None)
...
def command_read(args: argparse.Namespace) -> None:
    ...
    if args.all:
        ...
        tags = " or ".join(sorted(INTENSIVE_READING_TAGS))
        print(f"Prepared notes and extracts for {len(read_targets)} papers tagged {tags}.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name, func in [
        ...
        if name == "read":
            cmd.add_argument("--all", action="store_true", help="Prepare notes and extracts for all collection papers, not only intensive-reading-tagged papers.")
        if name == "sync":
