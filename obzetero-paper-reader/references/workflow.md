...
3. Run `read` to create notes and extract PDF text only for items tagged `待阅读`, `/待阅读`, `马上读`, `/马上读`, `阅读中`, or `/阅读中`; it still rebuilds the collection index and dashboard for the full collection.
