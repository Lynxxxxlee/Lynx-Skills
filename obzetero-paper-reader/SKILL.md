...
Only items with Zotero tag `待阅读`, `/待阅读`, `马上读`, `/马上读`, `阅读中`, or `/阅读中` are intensive-reading targets by default. Do not generate or rewrite `AUTO: Chinese Reading Notes` for papers without one of these tags unless the user explicitly requests `--all` or names those papers.

For Chinese intensive-reading content:

1. Ensure the target Zotero item has tag `待阅读`, `/待阅读`, `马上读`, `/马上读`, `阅读中`, or `/阅读中`.
